        ...
        <div class="cotea-bottom-shell">
          ...
          <div class="composer-row ${isActiveChipUnedited() ? 'caret-mode' : ''}">
            <div class="composer-input-wrap">
              <textarea id="question-input" rows="1" maxlength="${QUESTION_MAX_LENGTH}" placeholder="${escapeHtml(renderComposerPlaceholder())}" ${state.busy || !state.onProgrammers || state.languageNotSupported || !isComposerReady() ? 'disabled' : ''}>${escapeHtml(state.input)}</textarea>
            ...
            <button type="button" id="send-button" class="send-button" ${!state.input.trim() || state.busy || !state.onProgrammers || state.languageNotSupported || !isComposerReady() || isQuestionTooLong() ? 'disabled' : ''}>
...
async function handleSend() {
  ...
  if (!question || state.busy || !state.onProgrammers || state.languageNotSupported || !isComposerReady() || isQuestionTooLong()) {
    return;
  }
