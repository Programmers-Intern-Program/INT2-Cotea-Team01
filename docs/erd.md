...
> RAG 지식 베이스(B)는 벡터 DB 대상이라 이 ERD 범위에서 제외 — 별도 문서(벡터DB 스키마) 참고. (`common_pitfalls`는 2026-07-23에 백엔드 미연결 상태로 데이터·스키마 전체 제거.)
...
- **지식 베이스(B)**: 벡터 검색 대상이라 관계형 테이블이 아니라 벡터 DB 문서로 관리 — 별도 문서 참고
