...
| `approach.alternativeApproaches` | Lv2 | 이름만 나열, 각각을 길게 비교 설명하지 않기. 후보를 고를 때는 `classification.primary`로 매칭될 category/subcategory의 knowledge_base 문서(`rag/data_source/knowledge_base/{category}.json`)에 있는 `distinguishing_from[].ref` 목록을 참고할 것 — 그 문제에서 실제로 헷갈릴 만한 접근인지 사람이 판단해서 고르되, `distinguishing_from[].signal`(비교 설명문)은 여기 그대로 옮기지 않는다(런타임엔 이름만 노출되고 설명은 절대 노출되지 않으므로, `signal`은 작성자가 후보를 판단하는 근거로만 쓴다) |
...
- `common_pitfalls`는 2026-07-23에 데이터·빌드 스크립트·매핑을 전부 제거했다(백엔드에 한 번도 연결된 적이 없었고, 키워드 매칭 방식 자체가 Lv4에서 이미 `userCode` 원문을 보는 LLM 대비 실익이 낮다고 판단). 문제 데이터 쪽에서 신경 쓸 것 없음.
